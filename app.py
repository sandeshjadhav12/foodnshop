...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:////tmp/foodnshop.db'
...
@app.route('/udhari/summary')
@login_required
def udhari_summary():
    user = get_current_user()
    entries = UdhariEntry.query.filter_by(user_id=user.id).all()
    # Group by customer and compute net balance
    from collections import defaultdict
    balances = defaultdict(float)
    for e in entries:
        if e.entry_type == "debit":
            balances[e.customer_id] += e.amount
        else:
            balances[e.customer_id] -= e.amount
    total_due = sum(v for v in balances.values() if v > 0)
